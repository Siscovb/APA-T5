...
def decEstereo(ficCod, ficDec):
    ...
    y escribe el fichero 'ficDec' con los dos canales por separado en el formato de los ficheros WAVE estéreo.
    """
...
```
## También hemos creado otras funciones para que funcione correctamente

### Código de `Bits2fmt(BitsPerSample)`
``` python
def Bits2fmt(BitsPerSample):
    """ 
    Convierte los bits por píxel en formato WAVE 
    """
    if BitsPerSample == 8:
        return 'b'
...
### Código de `escrWave(ficWave, /, *, numChannels=2, SampleRate=44100, BitsPerSample=16, data=[])`
...
```
# Pruebas para verificar que funciona correctamente todas las funciones

```python
leeWave('wav\proba.wav')
estereo2mono('wav\proba.wav', 'wav\monoIzq.wav', canal=0) # Canal Izquierdo 'canal = 0'
estereo2mono('wav\proba.wav', 'wav\monoDer.wav', canal=1) # Canal Derecho 'canal = 1'
mono2estereo('wav\monoIzq.wav', 'wav\monoDer.wav', 'wav\estereo.wav')
codEstereo('wav\proba.wav', 'wav\codeEstereo.wav')
